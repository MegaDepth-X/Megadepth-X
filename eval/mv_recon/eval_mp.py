...
def main(hydra_cfg: DictConfig):
    ...
    for idx_model, model_keyname in enumerate(all_eval_models, start=1):
        ...
        for dataset_name in all_eval_datasets:
            ...
            if hasattr(all_datasets[dataset_name], "sequence_list"):
                seq_list_from_dataset = all_datasets[dataset_name].sequence_list
                print(f"Dataset {dataset_name} provides sequence_list with {len(seq_list_from_dataset)} sequences.")
                all_seq_ids = [item for item in all_seq_ids if item[1] in seq_list_from_dataset]
        ...
        # seq_ids_this_rank = seq_ids_this_rank[-3:-2]   # debug for a specific sequence
        tbar = tqdm(
            seq_ids_this_rank,
        )
        for dataset_name, seq_name, ids in tbar:
            # if "relif" in seq_name or "playground" in seq_name:
            #     continue
            ...
            if "images_processed" not in data:
                data['images_processed'] = data['images']
            ...
            pred_pts: np.ndarray   = infer_mv_pointclouds(filelist, model, hydra_cfg, (data_h, data_w), data.get("images_processed", None))  # (N, H, W, 3)
            ...
            # img_save_dir = osp.join(save_dir, f"{seq_name}_input_imgs")
            # os.makedirs(img_save_dir, exist_ok=True)
            # for idx_img in range(len(images)):
            #     imageio.imwrite(
            #         osp.join(img_save_dir, f"{idx_img:04d}.png"),
            #         (images[idx_img].permute(1, 2, 0).cpu().numpy() * 255).astype(np.uint8)
            #     )
            ...
            o3d.io.write_point_cloud(
                os.path.join(
                    save_dir, f"{seq_name.replace('/', '_')}-mask-icp.ply"
                ),
                pcd,
            )
