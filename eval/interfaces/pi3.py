...
from models.fastmodel import Pi3
from models.vggt.utils.geometry import closed_form_inverse_se3
from .graph_utils import chunk_batch, merge_predictions_torch, visualize_graph_with_images
...
def infer_monodepth(file: str, model: Pi3, hydra_cfg: DictConfig, image: Optional[torch.Tensor] = None):
    if image is not None:
        imgs = image.cuda().unsqueeze(0).unsqueeze(0)
    else:
...
def infer_videodepth(filelist: str, model: Pi3, hydra_cfg: DictConfig):
    ...
    depth_conf = torch.sigmoid(pred['conf'])[0, ..., 0]          # (N, h_14, w_14)
    return end - start, depth_map, depth_conf


def infer_cameras_w2c(filelist: str, model: Pi3, hydra_cfg: DictConfig, images: Optional[torch.Tensor] = None, blur_function = None):
    if images is not None:
        imgs = images.cuda().unsqueeze(0)
    else:
        imgs = load_and_resize14(filelist, new_width=hydra_cfg.load_img_size, device=hydra_cfg.device, verbose=hydra_cfg.verbose)
    ...
    if blur_function is not None:
        imgs = blur_function(imgs[0]).unsqueeze(0).to(imgs.dtype)
    ...
    local_points = pred['local_points'].cpu()
    confs = torch.sigmoid(pred['conf']).cpu()

    extrinsics = torch.tensor(
        closed_form_inverse_se3(np.array(poses_c2w_all[0]))
    )

    # return extrinsics, None, points[0], confs[0]
    rets = {
        "pred_extrs": extrinsics,
        "pred_c2ws": poses_c2w_all[0],
        "pred_intrs": None,
...
def infer_cameras_w2c_and_global_feat(filelist: str, model: Pi3, hydra_cfg: DictConfig, images: Optional[torch.Tensor] = None):
    if images is not None:
        imgs = images.cuda().unsqueeze(0)
    else:
        imgs = load_and_resize14(filelist, new_width=hydra_cfg.load_img_size, device=hydra_cfg.device, verbose=hydra_cfg.verbose)
    ...
    with torch.no_grad():
        with torch.amp.autocast(hydra_cfg.device, dtype=dtype):
            pred = model(imgs, get_global_feature_list=True)
    ...
    for idx in range(len(pred['global_feature_list'])):
        assert len(pred['global_feature_list'][idx]) == 1
    global_feature_list = [x[0] for x in pred['global_feature_list']]
...
def infer_cameras_c2w(filelist: str, model: Pi3, hydra_cfg: DictConfig, images: Optional[torch.Tensor] = None, blur_function = None):
    if images is not None:
        imgs = images.cuda().unsqueeze(0)
    else:
        imgs = load_and_resize14(filelist, new_width=hydra_cfg.load_img_size, device=hydra_cfg.device, verbose=hydra_cfg.verbose)
...
def infer_mv_pointclouds(filelist: str, model: Pi3, hydra_cfg: DictConfig, data_size: Tuple[int, int], images: Optional[torch.Tensor] = None, blur_function = None):
    if images is not None:
        imgs = images.cuda().unsqueeze(0)
    else:
        imgs = load_and_resize14(filelist, new_width=hydra_cfg.load_img_size, device=hydra_cfg.device, verbose=hydra_cfg.verbose)
