...
def load_and_preprocess_images_original(image_path_list, mode="crop"):
    ...
    for image_path in image_path_list:
        ...
        try:
            ...
            print(f"Error converting image to RGB {image_path}: {e}")
            continue

        width, height = img.size
        # print(f"Processing image {image_path} with original size (width={width}, height={height})")
        if mode == "pad":
            ...
            if width >= height:
                ...
                # If height is greater, set height to 518px
                new_height = target_size
...
def load_and_preprocess_images(image_path_list, new_width=518):
    """
    A quick start function to load and preprocess images for model input.
    This assumes the images should have the same shape for easier batching, but our model can also work well with different shapes.
